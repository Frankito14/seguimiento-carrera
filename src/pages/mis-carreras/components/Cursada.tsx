...
export default function CursadaCard({ id, nombre, universidad, materias }: Cursada) {
    ...
    return (
        <div className={`w-full bg-gray-50   rounded p-4 shadow-md text-neutral-700 border-1 border-neutral-300  transition`}>
            ...
            <p className="text-sm font-semibold">{universidad}</p>
            <p className="text-sm pt-2 font-semibold">Materias: <span className="text-green-700">{cantidadAprobadas}</span> / {cantidadMaterias}</p>
            <p className="text-sm pt-2 font-semibold">Progreso: <span className="text-green-700 font-bold">{porcentajeAprobacion}%</span></p>
        </div>
    )
}